...
import { fetchWithCache, getInitialCache, saveToCache, loadFromCache, removeFromCache } from '@/utils/fetchWithCache';
import { saveToCache as rawSave, loadFromCache as rawLoad } from "@/utils/localCache";
...
const AdminDashboard = () => {
      ...
      if (userId) {
        ...
        if (queue.length > 0) {
          for (const msg of queue) {
            try {
              await callBackendCompanion(msg.text);
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      const cacheKey = `chat_history_admin_${userId}_${conversationId}`; // Distinct key for admin if needed, or share? Assuming share for same user
      ...
      try {
        ...
        console.error('Failed to load messages from backend (cache also failed):', e);
      }
    };

    // Optimistic initial load
    const cached = getInitialCache(`chat_history_admin_${userId}_${conversationId}`);
  ...
  const callBackendCompanion = async (userMessage) => {
    ...
    try {
      ...
      if (!window.navigator.onLine) {
        setBotIsTyping(false);
        // Let sendMessage handle offline queue
        throw e;
      }
      setBotIsTyping(false);
    ...
    <nav className="space-y-2">
      {[
        ...
        <Button
          ...
          className={`w-full justify-start transition-all duration-300 hover:scale-105 group ${activeTab === key
