...
import { fetchWithCache, getInitialCache, saveToCache, loadFromCache, removeFromCache } from "@/utils/fetchWithCache"; // Use common utils
import { saveToCache as rawSave, loadFromCache as rawLoad } from "@/utils/localCache"; // Direct access if needed
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full space-y-6 p-8">
      <div className="text-center space-y-4">
        <div className={`w-32 h-32 mx-auto rounded-full flex items-center justify-center ${isConnected
...
const StudentDashboard = () => {
  ...
  // OFFLINE: Load queue on mount
  useEffect(() => {
    if (userId) {
      const queue = rawLoad(`pending_messages_${userId}`) || [];
      ...
      if (userId) {
        ...
        if (queue.length > 0) {
          for (const msg of queue) {
            // Try resending to backend logic (simplified)
            // In a real app we would call callBackendCompanion with the text
            // But we need to act as if user sent it.
            console.log('Syncing offline message:', msg);
            // For now, we just clear them to avoid duplicates (assuming user saw them locally)
            // ideally we re-post them to the API. 
            try {
              await callBackendCompanion(msg.text); // Try to get AI response now
            } catch (e) { console.error("Sync failed for msg", msg); }
          }
          rawSave(`pending_messages_${userId}`, []);
          setPendingMessages([]);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      // Offline: Cache Key
      const cacheKey = `chat_history_${userId}_${conversationId}`;

      try {
        // Use fetchWithCache wrapper
        ...
        // Merge with locally pending messages if any
        const localQueue = rawLoad(`pending_messages_${userId}`) || [];
        // Filter queue for this conversation only if we tracked convId, but for now show all user pending
        // Ideally we filter by conversationId.
        ...
        console.error("Message load error (using cache fallback if avail):", err);
        // If fetchWithCache failed completely (no cache), we might be here.
        // But fetchWithCache usually returns cache on error. 
        // If we are here, it means NO cache and NO network.
      }
    };

    // Optimistic initial load (before useEffect async)
    const cached = getInitialCache(`chat_history_${userId}_${conversationId}`);
    if (cached && messages.length === 0) {
      const formatted = (cached.messages || []).map(msg => ({
        id: msg.id || Date.now() + Math.random(),
        text: msg.message || msg.content || msg.text || '',
        isBot: msg.sender === 'assistant',
        timestamp: new Date(msg.created_at || msg.timestamp)
  ...
  const refreshConversations = async () => {
    ...
    try {
      ...
      console.log('[Conversations] Backend/Cache response:', data);
...
You are not alone, and there are people who want to help. Please reach out to one of these resources right now.`;
  ...
  const callBackendCompanion = async (userMessage) => {
    ...
    try {
      ...
      if (!window.navigator.onLine) {
        setBotIsTyping(false);
        // Queue the user message logic is handled in sendMessage for the *User* part
        // For the *Bot* part, we just return nothing or a placeholder?
        // Actually, if we are offline, the callBackendCompanion fails. 
        // We should probably catch this in sendMessage instead to avoid "Error".
        throw e; // Let sendMessage handle it
      }

      setBotIsTyping(false);
      // Fallback to direct ChatGPT if backend fails but we have net? 
      // or if offline fallback logic needs to kick in
      return null;
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!reply) {
        // Likely offline or failed silently
        if (!navigator.onLine) {
          // We are offline. User message is already in `messages`.
          // Add to pending queue to sync later
          console.log("Offline: Queueing message");
          const newQueue = [...pendingMessages, { text: trimmed, timestamp: Date.now() }];
          setPendingMessages(newQueue);
          rawSave(`pending_messages_${userId}`, newQueue);
          setIsLoading(false);
          return; // Stop here, no bot reply yet
        }
        // Online but failed? fallback
        setIsLoading(false);
        return;
      }
      ...
      if (!navigator.onLine) {
        console.log("Offline catch: Queueing message");
        const newQueue = [...pendingMessages, { text: trimmed, timestamp: Date.now() }];
        setPendingMessages(newQueue);
        rawSave(`pending_messages_${userId}`, newQueue);
      }
  ...
  const renderChatMessage = message => (
    ...
    >
      <div className="max-w-md animate-message-in">
        <div
          className={`p-4 rounded-2xl shadow-md ${message.isBot
          ...
          <div className="flex items-center space-x-2 mt-2">
            {['👍', '❤️', '😢', '💪', '🙏'].map(emoji => (
              <button
                ...
                className={`text-lg transition-all hover:scale-125 ${messageReactions[message.id] === emoji
      ...
      <CardHeader className="flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
                ...
                <div className="absolute right-0 top-12 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-3 w-64 z-50">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors ${reactionsEnabled ? 'bg-cyan-500' : 'bg-gray-300 dark:bg-gray-600'
                        }`}
                    >
                      <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${reactionsEnabled ? 'translate-x-5' : 'translate-x-0'
                        }`} />
                    </button>
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors ${incognitoMode ? 'bg-gray-700' : 'bg-gray-300 dark:bg-gray-600'
                        }`}
                    >
                      <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${incognitoMode ? 'translate-x-5' : 'translate-x-0'
                        }`} />
                    </button>
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
            ...
            <Badge
              className={`${isUsingChatGPT ? "bg-green-500" : "bg-orange-500"
                } text-white`}
            >
              {isUsingChatGPT
        ...
        <Tabs value={chatTab} onValueChange={setChatTab} className="chat-panel">
          ...
          <TabsContent value="chat" className="chat-panel">
            <div
              ...
              className={`chat-messages border rounded-xl ${theme.currentTheme === 'midnight'
                ? 'bg-slate-800'
            ...
            <div className="chat-input-bar bg-white dark:bg-gray-900">
              <div className="chat-input-inner relative">
                ...
                <button
                  ...
                  className={`w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0 flex items-center justify-center rounded-xl ${showEmojiPicker
                    ? "bg-yellow-400"
                ...
                <button
                  ...
                  className={`w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0 flex items-center justify-center rounded-xl ${isLiveTranscribing
                ...
                <div className="space-y-3">
                  {conversationHistory.map(chat => (
                    <div
                      ...
                      className={`p-4 border rounded-xl mb-3 cursor-pointer transition-all hover:shadow-md ${theme.currentTheme === 'midnight'
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div className="flex-1 min-w-0">
                          <p className={`font-semibold text-sm mb-1 ${theme.currentTheme === 'midnight' ? 'text-gray-200' : 'text-gray-800'
                            }`}>{chat.title}</p>
  ...
  const renderOverview = () => (
    <div className="space-y-6">
      ...
      {/* Online-Only Active Tasks Section */}
      <ActiveTasks
        theme={theme}
        onStartAssessment={(task) => {
          // Switch to assessments tab
          setActiveTab('assessments');
          // In a fuller implementation, we would pass 'task' to pre-select it
        }}
      />
    ...
    <nav className="space-y-2">
      {[
        ...
        <Button
          ...
          className={`w-full justify-start ${activeTab === key
            ? ""
            : theme.colors.text
            }`}
        >
