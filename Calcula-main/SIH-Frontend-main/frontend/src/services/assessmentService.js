...
/**
 * Get available assessment forms (Static + Dynamic)
 * @returns {Promise<{assessments: Array, total: number}>}
 */
export const getAvailableAssessments = async () => {
  const response = await apiClient.get('/student/assessments/available');
  return response.data.data;
};

/**
 * Create a new dynamic assessment form (Admin)
 * @param {object} formData - Form definition including questions and validity
 * @returns {Promise<object>}
 */
export const createAssessmentForm = async (formData) => {
  const response = await apiClient.post('/admin/assessments', formData);
  return response.data.data;
};
